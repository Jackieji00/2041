...
let rec remove_key map key =
  ...
  | Node(node) ->
    let diff = String.compare key node.key in
    if diff = 0 then
      if (node.left = Empty)&&(node.right = Empty) then   (*it is a leaf*)
        Empty
      else if (node.left = Empty) then    (*if it has a right child*)
        node.right
      else if (node.right = Empty) then   (*if it has a left child *)
        node.left
      else                                (*if it has 2 child*)
        let rec help map =
          match map with
          | Empty -> Empty
          | Node(node1)->
            if node1.left=Empty then
              begin
                let rest = remove_key node.right node1.key in      (*get rid of the successor*)
                Node{key=node1.key;value=node1.value;left=node.left;right=rest}  (*return succesor*)
              end
            else
              help node1.left
        in help node.right
    else if diff < 0 then         (*if key is smaller*)
