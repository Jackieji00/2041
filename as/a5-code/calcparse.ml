...
and parse_compare toks =
  ...
  | LessThan :: back ->
    let (rexpr,restf) = parse_addsub back in
    (Intop{op=Less;lexpr;rexpr},restf)
  | GreatThan :: back ->
    let (rexpr,restf) = parse_addsub back in
    (Intop{op=Greater;lexpr;rexpr},restf)
  | Equal :: back ->
    let (rexpr,restf) = parse_addsub back in
    (Intop{op=Equal;lexpr;rexpr},restf)
  | _ -> (lexpr, rest)
...
and parse_lambda toks =
  ...
  match toks with
  | At :: Ident varname :: rest->
    begin
      let (rexpr,rest) = parse_expr rest in
      (Lambda{param_name=varname;code_expr=rexpr},rest)
    end
  | At :: rest ->
