...
type expr =
  ...
  | Mul of expr * expr
  | Const of int
(* ADD IN variants for subtraction, division, variable identifiers *)
;;
...
let parse_tokens tokens =
  ...
  and prec1 toks =
    ...
    match rest with
    | Times :: tail ->                       (* * is first *)
       let (rexpr,rest) = prec1 tail in      (* recursively generate right-had expression *)
       (Mul(lexpr,rexpr), rest)              (* return multiplication of these two *)
    (* ADD IN a case for division *)
    | _ -> (lexpr, rest)                     (* not a multiply so return expression and remaining tokens *)

  (* prec2: addition and subtraction *)
  and prec2 toks =
    let (lexpr, rest) = prec1 toks in        (* try higher precdence expression first *)
    match rest with
    | Plus :: tail ->                        (* + is first *)
       let (rexpr,rest) = prec2 tail in      (* recursively generate right-had expression *)
       (Add(lexpr,rexpr), rest)              (* return addition of these two *)
    (* ADD IN a case for subtraction *)
